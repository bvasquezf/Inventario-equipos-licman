...
/**
 * Formulario de registro de equipos con fieldsets y radio cards.
 * Props:
...
export default function FormView({ bodegaInicial = '', onGuardar }) {
  ...
  return (
    <form
      ...
      className="rounded-[14px] border border-slate-200 bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.10)] sm:p-6"
      noValidate
    >
      <header className="mb-5">
        <h2 className="text-[1.2rem] font-bold text-slate-900">Nuevo registro</h2>
        <p className="text-sm text-slate-500">
          Completa los datos del equipo. El correlativo se asigna al guardar.
        </p>
      </header>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <label className="block text-[0.88rem] font-semibold text-slate-900">
            ...
            {errores.numero_interno && (
              <p className="mt-1 text-xs font-medium text-red-600">{errores.numero_interno}</p>
            )}
          </label>
          <label className="block text-[0.88rem] font-semibold text-slate-900">
            Número de serie
            ...
            {errores.numero_serie && (
              <p className="mt-1 text-xs font-medium text-red-600">{errores.numero_serie}</p>
            )}
          </label>
        </div>
          ...
          {ESTADOS.map((estado) => {
            ...
            return (
              ...
              >
                ...
                />
                <span>
                  <strong className={`block text-[0.92rem] ${checked ? 'text-blue-600' : 'text-slate-900'}`}>
      ...
      <div className="mt-4 flex flex-col gap-2.5 sm:flex-row">
        ...
        <button
          type="submit"
          disabled={guardando}
          className="flex-1 rounded-[10px] bg-blue-600 px-4 py-3.5 text-base font-bold text-white shadow-[0_4px_12px_rgba(37,99,235,0.3)] transition hover:bg-blue-700 disabled:cursor-wait disabled:opacity-70"
        >
